...
print(f"Unique countries in train: {train['country'].nunique()}")
print(f"Unique countries in test: {test['country'].nunique()}")
...
# Map test countries to team codes
test['team_code'] = test['ID'].apply(extract_country_code)
...
# Map stage names to simplified versions for prediction
stage_mapping = {
    'group stage': 'group',
    ...
    'quarter-finals': 'qf',
    'semi-finals': 'sf',
    'third-place match': 'runnerup',  # Treat as runner-up
    'final': 'champion'  # Winners
}
...
# Rule-based adjustments based on historical performance
pred_stage = []
for idx in range(len(X_test)):
    probs = pred_stage_proba[idx]
    best_stage = test_features.iloc[idx]['best_stage']
    appearances = test_features.iloc[idx]['appearances']
    championships = test_features.iloc[idx]['championships']
    semifinals = test_features.iloc[idx]['semifinals']
    
    # Get base prediction
    base_pred = np.argmax(probs)
    
    # Apply rules for strong teams
    if championships >= 2 or (championships >= 1 and semifinals >= 3):
        # Historical champions - boost chances of deep run
        if probs[base_pred] < 0.4:  # If not confident
            # Consider advancing to at least QF
            qf_idx = np.where(le_stage.classes_ == 'qf')[0]
            if len(qf_idx) > 0 and probs[qf_idx[0]] > 0.15:
                base_pred = qf_idx[0]
    
    elif best_stage >= 4 and semifinals >= 2:
        # Teams with strong history - boost chances
        if probs[base_pred] < 0.3:
            ro16_idx = np.where(le_stage.classes_ == 'roundof16')[0]
            if len(ro16_idx) > 0 and probs[ro16_idx[0]] > 0.15:
                base_pred = ro16_idx[0]
    
    elif appearances == 0:
        # New teams - likely group stage
        group_idx = np.where(le_stage.classes_ == 'group')[0]
        if len(group_idx) > 0:
            base_pred = group_idx[0]
    
    pred_stage.append(base_pred)

pred_stage = np.array(pred_stage)
pred_stage_names = le_stage.inverse_transform(pred_stage)

print(f"Stage predictions distribution:")
unique, counts = np.unique(pred_stage_names, return_counts=True)
...
submission = pd.DataFrame({
    ...
    'Target': pred_stage_names
})

# Ensure proper stage names match sample submission format
